...
with date_range as (
    ...
    -- One row per day for the last 8 months through today.
    select
        cast(current_date - interval 8 month as date) + interval (i) day as date_day
    from range(
        0,
        (current_date - cast(current_date - interval 8 month as date))::int + 1
    ) as t(i)
